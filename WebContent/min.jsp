...
String from = request.getParameter("from");
                ...
                PreparedStatement sc = conn.prepareStatement("select * from flight where splace=? and destination=? and f_id in(select f_id from ticket where cost=(select min(cost) from (select f_id, cost from ticket where f_id in (select f_id from flight where f_id in(select f_id from (select f_id , min(cost) as a from ticket group by f_id)) and splace=? and destination=?))))");
...
<td><%=rc.getString("f_id")%></td>
<td><%=rc.getString("name")%></td>
<td><%=rc.getString("destination")%></td>
<td><%=rc.getString("splace")%></td>
<td><%=rc.getString("day")%></td>
</tr>
</table><%} %>
                </center></b>

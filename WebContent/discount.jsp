...
<% 
 String f_id = request.getParameter("f_id");
                ...
                PreparedStatement sb = conn.prepareStatement("select distinct(cost) from ticket where f_id=? and type='BUSINESS'");
                PreparedStatement se = conn.prepareStatement("select distinct(cost) from ticket where f_id=? and type='ECONOMY'");
                sb.setString(1, f_id);
                se.setString(1, f_id);
                ResultSet rb = sb.executeQuery();
 ...
 int b=rb.getInt("cost");
 int e=re.getInt("cost");
 e=e-(e/10);
 b=b-(b/10);
 
 PreparedStatement pe=conn.prepareStatement("update ticket set cost=? where f_id=? and type='ECONOMY'");
 PreparedStatement pb=conn.prepareStatement("update ticket set cost=? where f_id=? and type='BUSINESS'");
 pe.setInt(1, e);
 pb.setInt(1, b);
 pe.setString(2, f_id);
 pb.setString(2, f_id);
 ResultSet rre = pe.executeQuery();
 ResultSet rrb = pb.executeQuery();
 
 %><br>The new price for Economy is :<% out.println(e); %><br>
 The new price for Business is <% out.println(b); %><br>
 </div>

...
try{
	...
	Statement jk=con.createStatement();
	String strQuery = "SELECT COUNT(*) FROM reservation";
	ResultSet tl = jk.executeQuery(strQuery);
	String Countrow="";
	while(tl.next()){
	Countrow = tl.getString(1);
	
	}
	int x=0;
 	PreparedStatement pd=con.prepareStatement("select t_id from ticket where f_id =? and type=? and status=?");
	pd.setString(1, fid);
	pd.setString(2,type);
		pd.setInt(3,0);
	ResultSet p = pd.executeQuery();
		p.next();
 	
	int n=p.getInt("t_id");
	
  	PreparedStatement ps = con.prepareStatement("insert into reservation values(?,?,?,?)");
	ps.setString(3,Countrow);
	ps.setString(2, fid);
	ps.setInt(4,n);
	ps.setString(1,pid);
	...
	if(rr.next()){
		out.println("your ticket was booked");%>
		<br>Your ticket ID is<%out.println(n); 
		PreparedStatement k=con.prepareStatement("update ticket set status=1 where t_id=?");
		k.setInt(1, n);
		ResultSet f=k.executeQuery();
		f.next();
	}
	else{
